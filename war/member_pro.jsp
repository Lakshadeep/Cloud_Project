...
<title>Team Member Projects</title>
...
out</a></td>
  ...
  <!-- end #sidebar1 --></div>
  <div id="mainContent">
    <h1>Projects you are member of</h1>
   <% 
	String url = null;
	...
	ResultSet rs = conn.createStatement().executeQuery("SELECT *  FROM Project_teams where User_id = '"+ s +"' and Role != 'mentor';");
	if(!rs.next()){
		%>
		<table width="80%" border="0"cellspacing="4" cellpadding="3px" >
  		<tr>
    	<td bgcolor="#CCCCFF">Currently you are not part of  any project.</td>
    	</tr>
		...
		rs.beforeFirst();
		while(rs.next()){
		ResultSet rs1 = conn.createStatement().executeQuery("SELECT *  FROM Projects where Id="+ rs.getInt("Project_id") +";");
		while(rs1.next()){
...
%>
    <form action="member_console.jsp?name=<%out.print(rs1.getString("Id"));%>&#s12s23fls" method="post">
    <table width="80%" border="0"cellspacing="4" cellpadding="3px" >
  <tr>
    <td bgcolor="#CCCCFF" width="80%">&nbsp;<% out.print(name); %></td>
    <td bgcolor="#CCCCFF" width="20%"><input type="submit" value="Project Console" /></td>

    </tr>
    <tr><td bgcolor="#CCCCCC" colspan="2">&nbsp;<% out.print(description); %></td>
